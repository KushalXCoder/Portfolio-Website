...
const ExperienceBox = ({logo, title, text, date}) => {
  return (
    ...
    >
        <div className="top-section w-full flex lg:justify-between lg:items-center max-sm:flex-col max-sm:gap-3">
            <div className="top-section-left flex gap-3 items-center">
                <img src={logo} alt={`${logo} Image`} className='h-10'/>
                <h1 className='text-xl font-bold max-sm:text-[16px] max-sm:w-full'>{title}</h1>
            </div>
            <div className="top-section-right max-sm:px-3">
                <h1>{date}</h1>
            </div>
        </div>
